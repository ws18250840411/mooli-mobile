...
import { times, range, padZero, getTrueValue, getMonthEndDay } from './lib/utils';
import Picker from '../picker';
var currentYear = new Date().getFullYear();
var DatetimePicker = /*#__PURE__*/function (_React$PureComponent) {
      ...
      if (type === 'time') {
        ...
        values = _this.originColumns.map(function (column) {
          switch (column.type) {
            ...
            case 'month':
              return formatter && formatter('month', padZero(value.getMonth() + 1));
            case 'day':
              return formatter && formatter('day', padZero(value.getDate()));
            case 'hour':
              return formatter && formatter('hour', padZero(value.getHours()));
            case 'minute':
              return formatter && formatter('minute', padZero(value.getMinutes()));
            default:
              return null;
          }
        });
  ...
  _createClass(DatetimePicker, [{
    ...
    value: function render() {
      ...
      var componentClassName = createClassName('date-picker');
